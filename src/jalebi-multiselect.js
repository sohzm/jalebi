    ...
    createView() {
        const optionsHTML = Array.from(this.querySelectorAll(":scope > option, :scope > optgroup")).map(el => {
            if (el.tagName === "OPTGROUP") {
                const groupOptions = Array.from(el.children).map(opt => {
                    return `<div class="option" data-value="${opt.value}">
                                ${opt.textContent}
                                ${this._values.includes(opt.value) ? `<svg width="16px" height="16px" stroke-width="2" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" color="var(--fg-2)">
            ...
            return `<div class="option" data-value="${el.value}">
                        ${el.textContent}
                        ${this._values.includes(el.value) ? `<svg width="16px" height="16px" stroke-width="2" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" color="var(--fg-2)">
                            <path d="M5 13L9 17L19 7" stroke="var(--fg-2)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                        </svg>` : ""}
                    </div>`;
        }).join("");
